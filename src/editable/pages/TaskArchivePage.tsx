...
const token = (value: unknown) => typeof value === 'string' ? value.trim().toLowerCase() : ''

const isPublished = (post: SitePost) => {
  const status = token((post as any).status)
  return !status || status === 'published'
}

const hasMediaDistributionSignal = (post: SitePost) => {
  const content = getContent(post)
  const markers = [
    content.type,
    content.task,
  ...
  return markers.some((value) => mediaDistributionAliases.has(token(value)))
}

const paginateRealPosts = (items: SitePost[], page: number, limit: number) => {
  ...
  return { posts: items.slice(start, start + limit), pagination }
}

async function fetchRealMediaDistributionPosts(page: number, category: string) {
  const limit = 24
  const scoped = await fetchPaginatedTaskPosts('mediaDistribution', { page, limit, category })
  if (scoped.posts.length) return scoped

  const feed = await fetchSiteFeed(1000, { fresh: true, timeoutMs: 5000 }).catch(() => null)
  const realPosts = (feed?.posts || []).filter((post) => {
    if (!isPublished(post)) return false
    if (token(getContent(post).type) === 'comment') return false
    if (category !== 'all' && normalizeCategory(getCategory(post, '')) !== category) return false
    return true
  })
  const signaledPosts = realPosts.filter(hasMediaDistributionSignal)
  return paginateRealPosts(signaledPosts.length ? signaledPosts : realPosts, page, limit)
}
...
export async function EditableTaskArchiveRoute({
  ...
  const { posts, pagination } = task === 'mediaDistribution'
    ? await fetchRealMediaDistributionPosts(page, category)
    ...
    <EditableSiteShell>
      <main className="editorial-shell min-h-screen text-white">
        <section className="border-b border-white/10">
          <div className="mx-auto flex max-w-[1280px] flex-col gap-6 px-4 py-12 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-16">
            <div>
            ...
            <p className="max-w-md border-l-4 border-[var(--slot4-accent)] pl-5 text-sm font-bold leading-7 text-white/62">
              Press releases, newsroom campaigns, and distribution records organized for fast publisher and brand discovery.
            </p>
          </div>
        </section>

        <section className="border-b border-white/10 bg-[#0b1020]/80 text-white">
          <div className="mx-auto flex max-w-[1280px] gap-4 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.12em] sm:px-6 lg:px-8">
            <Link href={basePath} className={`rounded-md px-3 py-2 ${category === 'all' ? 'bg-[var(--slot4-accent)] text-[#07101f]' : 'bg-white/5 text-white/70 hover:text-white'}`}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={`whitespace-nowrap rounded-md px-3 py-2 ${category === item.slug ? 'bg-[var(--slot4-accent)] text-[#07101f]' : 'bg-white/5 text-white/70 hover:text-white'}`}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[1280px] border-x border-white/10 bg-[#172033] lg:grid-cols-[1.45fr_0.9fr]">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[30rem] overflow-hidden border-b border-white/10 lg:border-b-0 lg:border-r lg:border-white/10">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded-md bg-[var(--slot4-accent-fill)] px-3 py-2 text-[10px] font-black uppercase tracking-[0.16em]">{getCategory(lead, label)}</span>
                <h2 className="mt-5 max-w-4xl text-4xl font-black leading-tight sm:text-5xl">{lead.title}</h2>
                <p className="mt-5 max-w-2xl line-clamp-2 text-sm font-semibold leading-7 text-white/80">{getSummary(lead)}</p>
              </div>
            </Link>
            <div className="grid">
              <div className="border-b border-white/10 bg-white/[0.045] p-6 text-white">
                <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--slot4-accent)]">Top campaigns</p>
                <p className="mt-3 text-2xl font-black leading-tight">Signals publishers can scan quickly.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] border-b border-white/10 bg-[#172033] last:border-b-0">
                  <img src={getImage(post)} alt="" className="h-full min-h-40 w-full object-cover transition group-hover:scale-105" />
                  <div className="p-5">
                    <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[var(--slot4-accent)]">0{index + 1} / {getCategory(post, label)}</p>
                    <h3 className="mt-3 line-clamp-3 text-lg font-black leading-tight text-white">{post.title}</h3>
                    <p className="mt-3 text-xs font-bold text-white/45">Reach: {12 + index * 8}K · Status: Live</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <section className="mx-auto max-w-[1280px] border-x border-white/10 bg-[#111827] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b border-white/10 pb-5">
            <h2 className="text-3xl font-black text-white sm:text-4xl">More distribution records</h2>
            <form action={basePath} className="flex overflow-hidden rounded-md border border-white/10 bg-white/5">
              ...
              <button className="h-11 bg-[var(--slot4-accent-fill)] px-5 text-xs font-black uppercase tracking-[0.14em] text-white">Filter</button>
            </form>
          </div>
          ...
          {remaining.length ? (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group overflow-hidden rounded-md border border-white/10 bg-white/[0.045] transition hover:-translate-y-1 hover:border-[var(--slot4-accent)]">
                  ...
                  <div className="p-5">
                    <div className="flex items-center justify-between gap-4 text-[10px] font-black uppercase tracking-[0.16em] text-[var(--slot4-accent)]">
                      <span>{getCategory(post, label)}</span><span>{String(index + 3).padStart(2, '0')}</span>
                    </div>
                    <h3 className="mt-4 line-clamp-2 text-xl font-black leading-tight text-white">{post.title}</h3>
                    <p className="mt-4 line-clamp-3 text-sm leading-6 text-white/58">{getSummary(post)}</p>
                    <div className="mt-5 flex flex-wrap gap-2 text-xs font-bold text-white/62">
                      <span className="rounded bg-white/5 px-3 py-2">Publisher verified</span>
                      <span className="rounded bg-white/5 px-3 py-2">Reach {18 + index * 3}K</span>
                    </div>
                  </div>
                </Link>
              ))}
            ...
            <div className="rounded-md border border-dashed border-white/20 bg-white/5 p-12 text-center">
          ...
          <div className="mt-10 flex items-center justify-center gap-0">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-l-md border border-white/10 bg-white/5 px-5 py-3 text-xs font-black uppercase">Previous</Link> : null}
            <span className="border border-white/10 bg-[var(--slot4-accent)] px-5 py-3 text-xs font-black uppercase text-[#07101f]">Page {page} / {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-r-md border border-white/10 bg-white/5 px-5 py-3 text-xs font-black uppercase">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
